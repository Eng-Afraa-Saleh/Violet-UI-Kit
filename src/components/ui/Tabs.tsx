...
export function TabTrigger({
  ...
  const baseClasses = cn(
    "inline-flex items-center justify-center font-medium transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed",
    size,
    className
  );
  ...
  const variantClasses = {
    ...
    segmented: cn(
      "flex-1 rounded-md px-3",
      isActive
        ? "bg-white text-slate-900 shadow-sm dark:bg-slate-800 dark:text-slate-50"
        : "text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-300"
    ),
    vertical: cn(
      "w-full justify-start rounded-lg px-4 py-2 text-left",
      isActive
...
export function TabContent({ value, className, children }: TabContentProps) {
  ...
  return (
    <div
      role="tabpanel"
      className={cn("py-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500", className)}
      tabIndex={0}
    >
      {children}
    </div>
